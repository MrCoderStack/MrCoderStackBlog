...
export class ArticleForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.catesArr) {
      this.setState({catesArr: nextProps.catesArr});
    }
  ...
  render() {
    ...
    const catesOption = [];
    this.state.catesArr.map((cate) => {
      catesOption.push(<Option key={cate}>{cate}</Option>)
  })
    // 封面上传组件
    const CoverUploader = (
  ...
  <Select
      ...
      onChange={this.handleCatesChange}
