  ...
  componentDidMount(props) {
    ...
    .then((response) => {
      this.setState({
        ...
        catesArr:response.data.catesArr,
      })
    })
    .catch((error) => {
      console.log(error);
    ...
    return (
      <div style={{padding:20}}>
        ...
        <Spin spinning={this.state.loading}>
          <ArticleForm article={this.state.article} tagsArr={this.state.tagsArr} catesArr={this.state.catesArr} isMarkdown={this.state.article.is_markdown} handleSubmit={this.handleSubmit.bind(this)}/>
        </Spin>
      </div>
    )
  }
